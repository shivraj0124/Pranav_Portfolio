...
function FieldError({ msg }: { msg?: string }) {
  if (!msg) return null;
  return (
    <p className="mt-1 text-xs text-red-400 font-medium pl-1 animate-fade-in">
      {msg}
    </p>
  );
}
    ...
    <div className="flex gap-3 justify-center">
      {[0, 1, 2, 3, 4, 5].map((i) => (
        <input
          ...
          onFocus={handleFocus}
          disabled={disabled}
          className={`w-14 h-14 text-center text-xl font-bold rounded-2xl border-2 outline-none transition-all duration-200 border-white/20 focus:border-[#0E49FF] focus:bg-[#0E49FF]/10 disabled:opacity-40 disabled:cursor-not-allowed ${dark ? "bg-white/5 text-white" : "bg-white text-black"}`}
...
function Contact({ dark }: { dark: boolean }) {
  ...
  const inputBase =
    "w-full rounded-2xl border px-5 py-4 text-sm outline-none transition-all duration-200 backdrop-blur-sm";
    ...
    <>
      ...
      <section id="contact" className={`relative overflow-hidden py-24 ${bg}`}>
        ...
        <div className="relative z-10 mx-auto max-w-2xl px-6 text-center">
          ...
          <p
            className={`mt-4 text-sm leading-relaxed ${dark ? "text-white/60" : "text-black/55"}`}
          >
          ...
          {step === "otp" && (
            <div className="mt-12 animate-slide-up">
              {/* Card */}
              <div
                className={`rounded-3xl border p-8 text-center ${dark ? "bg-white/5 border-white/10" : "bg-white border-[#0E49FF]/15"}`}
              >
                {/* Icon */}
                ...
                <p
                  className={`text-sm mb-1 ${dark ? "text-white/50" : "text-black/50"}`}
                >
                  We sent a 6-digit code to
                </p>
                <p className="text-sm font-bold text-[#0E49FF] mb-6">
                  {formData.email}
                </p>
                ...
                <div className="mt-5 flex items-center justify-between text-xs">
                  ...
                  >
                    {cooldown > 0
                      ? `Resend in ${formatExpiry(cooldown)}`
                      : "Resend OTP"}
                  </button>
                </div>
              </div>
          ...
          {step === "done" && (
            <div className="mt-12 animate-slide-up">
              <div
                className={`rounded-3xl border p-10 text-center ${dark ? "bg-white/5 border-white/10" : "bg-white border-[#0E49FF]/15"}`}
              >
                ...
                <p
                  className={`text-sm leading-relaxed mb-6 ${dark ? "text-white/55" : "text-black/55"}`}
                >
                  Thanks for reaching out. I will get back
                  <br />
